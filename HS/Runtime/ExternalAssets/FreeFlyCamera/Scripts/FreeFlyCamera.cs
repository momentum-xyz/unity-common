...
public class FreeFlyCamera : MonoBehaviour
{
    ...
    [SerializeField] bool _onlyRotateWithRMB = false; // Only capture the mouse when the right mouse button is held down
    ...
    {
        if (_active)
            _wantedMode = _onlyRotateWithRMB ? CursorLockMode.None : CursorLockMode.Locked;
    }
    ...
    private void SetCursorState()
    {
        ...
        if (Input.GetMouseButtonDown(1))                // TODO : find better fix for this - left click on components triggers this, even if it's a button, right click now activates this
        {
            _wantedMode = CursorLockMode.Locked;
        }
        else if( Input.GetMouseButtonUp(1) && _onlyRotateWithRMB )
        {
